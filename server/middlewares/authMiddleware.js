...
module.exports = function (req, res, next) {
    ...
    try {
        ...
        if (authorizationHeader === undefined) {
            return res.status(401).json({message: "Пожалуйста, укажите заголовки запроса для авторизации"})
        }
        const token = authorizationHeader.split(' ')[1] // Bearer ...absfu_TOKEN_absfu...
        if (!token) {
            return res.status(401).json({message: "Не авторизован (отсутствие токена)"})
        }
